...
export const getCategories = async () => {
  if (USE_MOCK) {
    const { MOCK_CATEGORIES } = await import('@/mock/applications');
...
export const createCategory = async (payload) => {
  if (USE_MOCK) return mock({ id: `CAT-${Date.now()}`, ...payload, status: 'ACTIVE', createdAt: new Date().toISOString() });
