...
export default function ApprovalMessagesPage() {
  ...
  const messagesWithStatus = (data ?? []).filter((app) => 
              ...
              <div className="card-body">
                ...
                <div className={styles.messageDetails}>
                  <div className={styles.detailGrid}>
                    <MessageField label="Gosa" value={typeof app.category === 'string' ? app.category : app.category?.name || '—'} />
...
function getMessageContent(app) {
  ...
  if (status === APPLICATION_STATUS.APPROVED) {
    return (
      <div className={`${styles.messageBox} ${styles.messageSuccess}`}>
        ...
        {reviewComments && <p className={styles.messageComment}>"{reviewComments}"</p>}
        <div className={styles.messageAction}>
          <p className={styles.nextSteps}>
            <strong>Next Steps:</strong> Your official business certificate will be generated and available shortly.
          </p>
        </div>
      </div>
    );
  ...
  if (status === APPLICATION_STATUS.PERMISSION_APPROVED) {
    return (
      <div className={`${styles.messageBox} ${styles.messageInfo}`}>
        ...
        <p>Your business permits have been reviewed and approved by the Commercial Office.</p>
        <p className={styles.messageComment}>{reviewComments || 'Your documentation meets all commercial requirements.'}</p>
        <p className={styles.nextSteps}>
          <strong>Status:</strong> Your application is now being reviewed by Addaf Turizm Biro for language compliance.
        </p>
      </div>
    );
  }
  ...
  if (status === APPLICATION_STATUS.LANGUAGE_REVIEW_PENDING) {
    return (
      <div className={`${styles.messageBox} ${styles.messageInfo}`}>
        <div className={styles.messageBoxHeader}>
          <span className={styles.messageBoxIcon}>🔍</span>
          <h4>Under Review - Addaf Turizm Biro</h4>
        </div>
  ...
  if (status.includes('REJECTED')) {
    ...
    const department = isPermissionRejected ? 'Commercial Office' : 'Addaf Turizm Biro';
  ...
  if (status === APPLICATION_STATUS.PERMISSION_PENDING) {
    return (
      <div className={`${styles.messageBox} ${styles.messageInfo}`}>
        <div className={styles.messageBoxHeader}>
          <span className={styles.messageBoxIcon}>⏳</span>
          <h4>Pending Review - Commercial Office</h4>
        </div>
        <p>Your application and business permits are being reviewed by the Commercial Office.</p>
        <p className={styles.nextSteps}>
  ...
  return (
    <div className={`${styles.messageBox} ${styles.messageInfo}`}>
      ...
      <p className={styles.nextSteps}>
        <strong>Status:</strong> Communication Biro is coordinating your application review.
      </p>
    </div>
  );
}
