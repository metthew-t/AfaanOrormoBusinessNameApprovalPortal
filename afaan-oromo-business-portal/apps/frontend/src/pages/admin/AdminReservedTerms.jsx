...
import { getReservedTerms, createReservedTerm, deleteReservedTerm } from '@/services/adminService';
...
export default function AdminReservedTerms() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Failed to add reserved term');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="page-content">
        ...
        <DataTable
          ...
          actions={(t) => (
            <Button
              variant="danger"
              size="sm"
              onClick={() => setDeleteDialog({ open: true, term: t, loading: false })}
            >
              Remove
            </Button>
          )}
      ...
      >
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
            ...
            <Button type="submit" loading={submitting}>Add Term</Button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmDialog
        ...
        title="Remove Reserved Term"
        message={
          <span>
            Are you sure you want to remove <strong>"{deleteDialog.term?.term}"</strong> from the reserved list?
