...
export default function AdminCategoriesPage() {
  ...
  const handleToggleStatus = async (category) => {
    try {
      ...
      await updateCategory(category.id, { isActive: newStatus });
  ...
  const columns = [
    ...
    { 
      ...
      label: 'Status', 
      render: (status, cat) => {
        const isActive = status === 'ACTIVE' || cat.isActive === true || cat.isActive === undefined;
        return (
          <button
    ...
    { key: 'createdAt', label: 'Created At', render: (date) => formatDate(date) },
    { key: 'updatedAt', label: 'Updated At', render: (date) => formatDate(date) },
    {
      key: 'actions',
      label: 'Actions',
      render: (_, category) => (
