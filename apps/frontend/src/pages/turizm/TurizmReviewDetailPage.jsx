...
export default function TurizmReviewDetailPage() {
  ...
  // Editable business name state
  const [editedBusinessName, setEditedBusinessName] = useState('');
  const [isEditingName, setIsEditingName] = useState(false);

  // Initialize edited name when app loads
  useEffect(() => {
    if (app?.businessName) {
      setEditedBusinessName(app.businessName);
    }
  }, [app]);
    ...
    try {
      if (action === 'approve') {
        await approve({ 
          applicationId: id, 
          comment: reason,
          suggestedBusinessName: editedBusinessName !== app.businessName ? editedBusinessName : undefined
        });
    ...
    <div className={styles.page}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.mainColumn}>
          {/* Editable Business Name Card */}
          <div className="card">
            <div className="card-header">
              <h2>Maqaa Daldaalaf Barbaadame (Proposed Business Name)</h2>
            </div>
            <div className="card-body">
              <Alert variant="info" style={{ marginBottom: 'var(--space-4)' }}>
                <strong>Language Officer Responsibility:</strong> You can review and edit the business name to ensure it meets Afaan Oromo language standards.
              </Alert>
              ...
              <Input
                ...
                placeholder="Enter corrected business name if needed..."
                hint={editedBusinessName !== app?.businessName ? "You have modified the business name. This suggestion will be included in your review." : "Original name from applicant"}
              />
              
              {editedBusinessName !== app?.businessName && (
                <div style={{ marginTop: 'var(--space-3)', padding: 'var(--space-3)', background: 'var(--color-warning-bg)', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-warning)' }}>
                  <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-warning-dark)', margin: 0 }}>
                    <strong>⚠️ Name Modified:</strong> Original: "{app?.businessName}" → Suggested: "{editedBusinessName}"
                  </p>
                </div>
              )}
            </div>
