            ...
            <div className="card-body">
              <div className={styles.documentBox}>
                ...
                <div className={styles.documentInfo}>
                  <h3>{app.permissionDocuments?.[0]?.fileName ?? 'Business Permission Certificate'}</h3>
                  <p className={styles.documentMeta}>
                    Uploaded: {formatDate(app.permissionDocuments?.[0]?.uploadedAt ?? app.submittedAt)}
                  </p>
                  <p className={styles.documentSize}>
                    {app.permissionDocuments?.[0]?.fileSize ? `${(app.permissionDocuments[0].fileSize / 1024 / 1024).toFixed(2)} MB` : 'Document attached'}
                  </p>
                </div>
                {app.permissionDocuments?.[0] && (
                  <Button 
                    variant="secondary" 
                    size="sm"
                    onClick={async () => {
                      try {
                        const token = localStorage.getItem('aobnap_token');
                        const response = await fetch(app.permissionDocuments[0].url, {
                          headers: {
                        ...
                        a.href = url;
                        a.download = app.permissionDocuments[0].fileName;
