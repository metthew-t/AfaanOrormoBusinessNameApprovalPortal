...
async function getQueue() {
  ...
  return permissions.map(p => ({
    ...
    permissionDocuments: (p.application.documents || []).map(doc => ({
      id: doc.id.toString(),
      fileName: doc.originalFilename,
      fileSize: doc.fileSize,
      uploadedAt: doc.createdAt,
      url: `/api/applications/${p.application.id}/documents/${doc.id}`,
    })),
...
async function getReviewDetail(permissionId) {
  ...
  return {
    ...
    permissionDocuments: (permission.application.documents || []).map(doc => ({
      id: doc.id.toString(),
      fileName: doc.originalFilename,
      fileSize: doc.fileSize,
      uploadedAt: doc.createdAt,
      url: `/api/applications/${permission.application.id}/documents/${doc.id}`,
    })),
